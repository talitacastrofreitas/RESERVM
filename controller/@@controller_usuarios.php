...
if ($_SERVER["REQUEST_METHOD"] == "POST" || $_SERVER["REQUEST_METHOD"] == "GET") {
  try {
    ...
    if ($acao === 'cadastrar') {
      ...
      //
      $sql = $conn->prepare("SELECT * FROM $view_colaboradores WHERE EMAIL = :EMAIL");
      $sql->bindParam(':EMAIL', $user_email);
      $sql->execute();
      $row = $sql->fetch(PDO::FETCH_ASSOC);
      if (!$row) {
        throw new Exception("Falha ao cadastrar os dados: E-mail não encontrado!");
      }

      $user_nome      = trim($row['NOMESOCIAL']);
      $user_matricula = trim($row['CHAPA']);
    ...
    if ($acao === 'cadastrar') {
      ...
      $sqlVerifica = "SELECT COUNT(*) FROM usuarios WHERE user_email = :user_email OR user_matricula = :user_matricula";
      $stmtVerifica = $conn->prepare($sqlVerifica);
      $stmtVerifica->execute([':user_email' => $user_email, ':user_matricula' => $user_matricula]);
      $existe = $stmtVerifica->fetchColumn();
      if ($existe > 0) {
        throw new Exception("Falha ao cadastrar os dados: E-mail já cadastrado!");
      }
    ...
    $_SESSION["form_user"] = $_POST; // CRIA SESSÃO PARA OS DADOS DO FORMULÁRIO QUE FORAM ENVIADOS
    header(sprintf('location: %s', $_SERVER['HTTP_REFERER']));
    exit;
  }
} else {
