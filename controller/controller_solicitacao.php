 ...
 "image/bmp",
   ...
   if ($result_etapa['solic_etapa'] != 3) {
    ...
    // 3. Lógica para definir o status e roteamento
        ...
        $STATUS_SOLICITADO_COORD = 2; // Roteia para Coordenador (ou SAAP, se não houver)
        $STATUS_APROVADO_COORD = 5;   // Roteia direto para SAAP (Pós Coordenador)
        // -----------------------------
    ...
    // Variáveis de Status e Roteamento
    $num_status = $STATUS_SOLICITADO_COORD; // Default: 2 (Solicitado)
    $sta_an_obs = NULL; // Observação de análise/status
    $is_pendente_coordenador = false; // Flag para notificar apenas Coordenadores

    if (!$has_coordenador) {
     // REGRA 1: Curso NÃO TEM Coordenador -> Vai direto para o SAAP como SOLICITADO (ID 2).
     $num_status = $STATUS_SOLICITADO_COORD; 
     $email_subject = 'Nova Solicitação';
     $pendencia_para = 'a Central de Análise (SAAP)';
     $link_destino = $url_sistema . "/admin/solicitacoes_submetidas.php"; // Link para a fila do SAAP (Todos)
    } elseif ($has_coordenador && $is_solicitante_coordenador) {
     // REGRA 2: Curso TEM Coordenador E o Solicitante É o Coordenador -> Vai direto para o SAAP como APROVADO PELO COORDENADOR (ID 5).
     $num_status = $STATUS_APROVADO_COORD; // Status 5
     $email_subject = "Nova Solicitação";
     $pendencia_para = 'Nova Solicitação';
     $link_destino = $url_sistema . "/admin/solicitacoes_submetidas.php"; // Link para a fila do SAAP
     $sta_an_obs = "Aprovação automática no envio, pois o solicitante é o Coordenador.";
    } elseif ($has_coordenador && !$is_solicitante_coordenador) {
     // REGRA 3: Curso TEM Coordenador E o Solicitante NÃO É o Coordenador -> Vai para o Coordenador como SOLICITADO (ID 2).
     $num_status = $STATUS_SOLICITADO_COORD; // Status 2
     $email_subject = "PENDÊNCIA: NOVA SOLICITAÇÃO";
     $pendencia_para = 'o Coordenador do Curso de <strong>' . htmlspecialchars($course_name) . '</strong>';
     $link_destino = $url_sistema . "/admin/solicitacoes_emAnalise.php"; // Link para a fila do Coordenador
     $is_pendente_coordenador = true; // Notifica apenas o Coordenador
    }

    // 3. Atualiza o Status principal e insere na Análise
