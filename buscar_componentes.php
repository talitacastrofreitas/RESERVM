...
//   try {
//     // Buscar componentes relacionados ao curso
//     $stmt = $conn->prepare("SELECT compc_id, compc_componente FROM componente_curricular WHERE compc_curso = ?");
//     $stmt->execute([$cursoId]);
//     $componentes = $stmt->fetchAll(PDO::FETCH_ASSOC);

//     if ($componentes) {
//       //echo '<option value="">Selecione um componente</option>';
//       echo '<option selected disabled value=""></option>';
//       foreach ($componentes as $componente) {
//         $selected = ($componente['compc_id'] == $_POST['componente_id']) ? 'selected' : '';
//         echo "<option value='{$componente['compc_id']}' $selected>{$componente['compc_componente']}</option>";
//       }
