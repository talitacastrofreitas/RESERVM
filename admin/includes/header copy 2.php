...
$count_pend_coord_total = $stmt_pend_coord_total->fetchColumn();
// DEBUG START
echo "";
// DEBUG END

// =================================================================================
// 3. CONTAGEM DE CANCELAMENTOS PENDENTES (Status 1)
$query_canc_pend = "SELECT COUNT(*) FROM solicitacao_cancelamento WHERE solcanc_status = 1";
$stmt_canc_pend = $conn->prepare($query_canc_pend);
$stmt_canc_pend->execute();
$count_canc_pend = $stmt_canc_pend->fetchColumn();

// TOTAL GERAL DE PENDÊNCIAS (Solicitado + Pendências Coordenador + Cancelamento)
$row_count_total = $count_solic_submetida + $count_pend_coord_total + $count_canc_pend;
?>
...
WHERE 
    ss.solic_sta_status IN (2, 3)";
                  ...
                  <?php
                  // Consulta para contar solicitações de cancelamento PENDENTES (status 1)
                  $query_canc_pend = "SELECT COUNT(*) FROM solicitacao_cancelamento WHERE solcanc_status = 1";
                  $stmt_canc_pend = $conn->prepare($query_canc_pend);
                  $stmt_canc_pend->execute();
                  $count_canc_pend = $stmt_canc_pend->fetchColumn();
                  ?>
                  <li class="nav-item">
                    <a href="canceladas.php" class="nav-link">
                      Cancelado
                      <?php if ($count_canc_pend > 0) { ?>
                        <div class="cont_sub"><?= $count_canc_pend ?></div>
                      <?php } ?>
                    </a>
                  </li>
