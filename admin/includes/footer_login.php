</div>
</div>
</div>
</div>

<footer class="footer_user_register mt-auto">
  <div class="row justify-content-center">
    <div class="col-sm-12">
      <img src="../assets/images/logo_bahiana_registro.svg" alt="">
      <p>&copy; <script>
          document.write(new Date().getFullYear())
        </script> RESERVM - Sistema de Reserva de Espaço Bahiana.<br>Todos os direitos reservados.</p>
    </div>
  </div>
</footer>

</div>
</div>

<!-- JAVASCRIPT -->
<script src="../assets/libs/bootstrap/js/bootstrap.bundle.min.js"></script>
<script src="../assets/libs/simplebar/simplebar.min.js"></script>
<script src="../assets/libs/node-waves/waves.min.js"></script>
<script src="../assets/libs/feather-icons/feather.min.js"></script>
<script src="../assets/js/pages/plugins/lord-icon-2.1.0.js"></script>
<script src="../assets/js/plugins.js"></script>
<!-- APP -->
<script src="../assets/js/app.js"></script>
<!-- VALIDA FORMULÁRIOS -->
<script src="../assets/js/valida_form.js"></script>

<!-- CONFIRMAÇÃO -->
<?php if (isset($_SESSION["msg"])) { ?>
  <script>
    Toastify({
      text: "<?= $_SESSION["msg"] ?>",
      duration: 5000, // Duração em milissegundos
      close: true, // Mostra o botão de fechar
      gravity: "top", // `top` ou `bottom`
      position: "right", // `left`, `center` ou `right`
      backgroundColor: "#38C172", // Cor de fundo
      stopOnFocus: true, // Para a duração ao passar o mouse
    }).showToast();
  </script>
<?php unset($_SESSION["msg"]);
} ?>
<<<<<<< HEAD
<!-- ==============================================SENHAEXPIRADA -->
<?php
// Adicionar esta lógica para capturar e exibir a mensagem da URL
if (isset($_GET["sucesso"])) {
    // Usamos urldecode para decodificar a mensagem
    $mensagem_url = htmlspecialchars(urldecode($_GET["sucesso"])); 
    ?>
    <script>
      Toastify({
        text: "<?= $mensagem_url ?>",
        duration: 5000,
        close: true,
        gravity: "top",
        position: "right",
        backgroundColor: "#38C172",
        stopOnFocus: true,
      }).showToast();
    </script>
    <?php 
    // Não precisa de unset, pois é uma variável GET
} 
?>

<script>
  // Função para limpar os parâmetros GET da URL
  function limparParametrosUrl() {
    // Verifica se há parâmetros na URL (ex: ?sucesso=...)
    if (window.location.search) {
      // Cria um novo objeto URL sem os parâmetros de busca
      var urlSemParametros = window.location.origin + window.location.pathname;

      // Substitui o estado atual do histórico do navegador
      // Isso muda a URL na barra de endereço sem recarregar a página
      window.history.replaceState({}, document.title, urlSemParametros);
    }
  }

  // Chama a função após um pequeno delay para garantir que o Toastify inicie
  // 500 milissegundos é geralmente suficiente
  if (window.location.search.includes('sucesso=')) {
      setTimeout(limparParametrosUrl, 500); 
  }
</script>
<!-- ============================================== -->
=======
>>>>>>> 37aa9c8fd78f14fcc367b2475fb179438e032599

<?php if (isset($_SESSION["erro"])) { ?>
  <script>
    Toastify({
      text: "<?= $_SESSION["erro"] ?>",
      duration: 5000, // Duração em milissegundos
      close: true, // Mostra o botão de fechar
      gravity: "top", // `top` ou `bottom`
      position: "right", // `left`, `center` ou `right`
      backgroundColor: "#C4453E", // Cor de fundo
      stopOnFocus: true, // Para a duração ao passar o mouse
    }).showToast();
  </script>
<?php unset($_SESSION["erro"]);
} ?>

<?php if (isset($_SESSION["rest"])) { ?>
  <script>
    Toastify({
      text: "<?= $_SESSION["rest"] ?>",
      duration: 5000, // Duração em milissegundos
      close: true, // Mostra o botão de fechar
      gravity: "top", // `top` ou `bottom`
      position: "right", // `left`, `center` ou `right`
      backgroundColor: "#F6993F", // Cor de fundo
      stopOnFocus: true, // Para a duração ao passar o mouse
    }).showToast();
  </script>
<?php unset($_SESSION["rest"]);
} ?>

<?php if (isset($_SESSION["atencao"])) { ?>
  <script>
    Toastify({
      text: "<?= $_SESSION["atencao"] ?>",
      duration: 5000, // Duração em milissegundos
      close: true, // Mostra o botão de fechar
      gravity: "top", // `top` ou `bottom`
      position: "right", // `left`, `center` ou `right`
      backgroundColor: "#F6993F", // Cor de fundo
      stopOnFocus: true, // Para a duração ao passar o mouse
    }).showToast();
  </script>
<?php unset($_SESSION["atencao"]);
} ?>

<script>
  function showPage() {
    document.body.style.opacity = "1"; // Mostra a página alterando a opacidade
  }
</script>
</body>

</html>