                ...
                <div class="etapa d-none" id="etapa3">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-6 col-lg-4 col-xl-3 campo-fixa-cad" style="display: none;">
                                <label class="form-label">Data Fim <span>*</span></label>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (modalReserva) {
            modalReserva.addEventListener('hidden.bs.modal', function () {
                const solicId = document.getElementById('btn_cadastrar_reserva').getAttribute('data-bs-solic_id');

                fetch('liberar_edicao.php', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        if (!data.success) {
                            console.error('Erro ao liberar a edição:', data.message);
                        }
                    })
                    .catch(error => {
...
<script>
    document.querySelector('#btnEditarSelecionados').addEventListener('click', function (event) {
        const modalEditt = document.getElementById('modal_edit_espaco');

        if (modalEditt) { // Verificação para garantir que o modal exista
            if (tipoReserva === "2") {
                const editDataInicioSemanal = modalEdit.querySelector('#edit_data_inicio_semanal');
                const editDataFimSemanal = modalEdit.querySelector('#edit_data_fim_semanal');
                if (editDataInicioSemanal) {
                    editDataInicioSemanal.closest('.col-6').style.display = 'none';
                }
                if (editDataFimSemanal) {
                    editDataFimSemanal.closest('.col-6').style.display = 'none';
                }
            } else {
                const editDataReserva = modalEdit.querySelector('#edit_data_reserva');
                if (editDataReserva) {
                    editDataReserva.closest('.col-6').style.display = 'none';
                }
            }

            const myModal = new bootstrap.Modal(modalEditt);
            myModal.show();
        } else {
