...
if ($_SERVER["REQUEST_METHOD"] == "POST" || $_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  try {
    ...
    if ($acao === 'cadastrar') {
      ...
      $res_ids = $_POST['res_id']; // Pode ser um único ID ou uma string de IDs
      $log_acao = 'Atualização';

      // Verificação para determinar se é edição em massa ou individual
      if (strpos($res_ids, ',') !== false) {
        ...
        // Se a edição em massa for apenas para reservas fixas, os campos a seguir
        // serão sempre os mesmos para todos os registros selecionados.
        $res_solic_id = $_POST['res_solic_id'];
        $res_campus = $_POST['res_campus'];
        $res_espaco_id = ($_POST['res_campus'] == 1) ? $_POST['res_espaco_id_cabula'] : $_POST['res_espaco_id_brotas'];
        ...
        $campos = [
          ...
          'res_campus' => $res_campus,
          'res_espaco_id' => $res_espaco_id,
          // Adicione os outros campos aqui com seus valores do POST
        ...
        foreach ($campos as $campo_db => $valor) {
          // Apenas adiciona ao UPDATE se o valor não for NULL, uma string vazia ou um array vazio.
          // Para 'res_recursos_add' (string de IDs), a verificação de empty pode ser útil.
          if ($valor !== null && $valor !== '' && (!is_array($valor) || !empty($valor))) {
            // Previne de campos como 'res_espaco_id_cabula' ou '_brotas' de entrar no update.
            // Usamos o nome da coluna do DB como chave do array.
            $updates[] = "{$campo_db} = ?";
            $params[] = $valor;
          }
        }
