      ...
      if (($query_admin) and ($query_admin->rowCount() != 0)) {
        ...
        if ($row_admin['nivel_acesso'] !== 2) { // SE O NÍVEL DE ACESSO DO ADMIN FOR '2', NÃO PODE ACESSAR. A CONTA FOI EXCLUÍDA
          ...
          if ($row_admin['admin_status'] == 1) { // SE O STATUS DO ADMINISTRADOR FOR 'INATIVO', NÃO PODE LOGAR
            ...
            if ($diferenca_data > 90 && $admin_matricula == $row_admin['admin_matricula'] && password_verify($dados['senha'], $row_admin['admin_senha']) == 1) {
    $admin_id_codificado = base64_encode($row_admin['admin_id']); // << ADICIONAR CODIFICAÇÃO
...
// =================================================================================
                // LÓGICA DE REDIRECIONAMENTO POR PERFIL (CORREÇÃO APLICADA)
                // =================================================================================
                $PERFIL_OPERADOR = 2; // ID do Perfil Operador
                
                if ((int)$row_admin['admin_perfil'] === $PERFIL_OPERADOR) {
                    // Se for Operador, vai para Reservas Confirmadas
                    header("Location: $url_sistema/admin/reservas_confirmadas.php");
                } else {
                    // Se for Administrador (Perfil 1) ou qualquer outro, vai para Solicitações Submetidas
                    header("Location: $url_sistema/admin/solicitacoes_submetidas.php");
                }
                // =================================================================================
...
$mensagem_sucesso = urlencode("Senha atualizada com sucesso!"); 

    // Redireciona para a página de login, passando a mensagem na URL
  header("Location: $url_sistema/admin?sucesso=$mensagem_sucesso");
  exit();

    // echo "<h1>TESTE DE REDIRECIONAMENTO DE SUCESSO</h1>";
    // echo "<p>O sistema está tentando redirecionar para:</p>";
    // echo "<h2>" . $url_sistema . "/admin</h2>";
    // exit();
  } catch (PDOException $e) {
    $conn->rollBack();
