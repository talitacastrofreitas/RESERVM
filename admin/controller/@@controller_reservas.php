...
if ($_SERVER["REQUEST_METHOD"] == "POST" || $_SERVER["REQUEST_METHOD"] == "GET") {
  try {
    ...
    if ($acao === 'cadastrar' || $acao === 'atualizar') {
      ...
      // POST
      $res_id_base = bin2hex(random_bytes(16)); // base para gerar IDs únicos por data

      // FUNÇÃO PARA VERIFICAR SE O CÓDIGO JÁ EXISTE
      function verificarCodigoNoBanco($res_codigo, $conn)
      {
        $stmt = $conn->prepare("SELECT COUNT(*) FROM reservas WHERE res_codigo = :res_codigo");
        $stmt->bindParam(':res_codigo', $res_codigo);
        $stmt->execute();
      ...
      $sql = "SELECT semp_data_inicio, semp_data_fim FROM conf_semestre_periodo";
      $stmt = $conn->query($sql);
