    ...
    if ($acao === 'cadastrar') {
      ...
      if (strpos($res_ids, ',') !== false) {
        ...
        $campos = [
          ...
          'res_data_inicio_semanal' => $res_tipo_reserva == 2 ? $_POST['res_data_inicio_semanal'] : NULL,
          'res_data_fim_semanal' => $res_tipo_reserva == 2 ? $_POST['res_data_fim_semanal'] : NULL,
          'res_dia_semana' => $res_tipo_reserva == 2 ? $_POST['res_dia_semana_fixa'] : $_POST['res_dia_semana'] ?? null,
          'res_data' => $res_tipo_reserva == 1 ? $_POST['res_data'] : NULL,
        ...
        foreach ($campos as $campo_db => $valor) {
        ...
        if (!empty($updates)) {
          ...
          $sql = "UPDATE reservas SET {$update_sql} WHERE res_id IN ({$placeholders})";
          $stmt = $conn->prepare($sql);
          $stmt->execute($params);
        }

        header('Location: ../../solicitacao_analise.php?i=' . $_POST['res_solic_id'] . '#ancora_reservas_confirmadas');
        exit();

      } else {
        // Lógica de Edição Individual
        $res_id = $_POST['res_id'];
        $log_acao = 'Atualização';
        ...
        $stmt->execute([
          ...
          ':res_tipo_reserva' => $res_tipo_reserva,
          ':res_data' => $res_tipo_reserva == 1 ? $res_data_diaria : NULL,
          ':res_mes' => $res_tipo_reserva == 1 ? $res_mes_diaria : NULL,
          ':res_ano' => $res_tipo_reserva == 1 ? $res_ano_diaria : NULL,
          ':res_dia_semana' => $res_tipo_reserva == 1 ? $res_dia_semana_diaria : $res_dia_semana_fixa,
          ':res_data_inicio_semanal' => $res_tipo_reserva == 2 ? $res_data_inicio_semanal : NULL,
          ':res_data_fim_semanal' => $res_tipo_reserva == 2 ? $res_data_fim_semanal : NULL,
