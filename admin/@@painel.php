  ...
  <div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        ...
        <form id="formBusca">
          <div class="row row-cols-1 row-cols-sm-2 row-cols-md-6 row-cols-xxl-6 g-3">
...
<?php
try {
  // Obtenção dos dados da tabela `salas` para a data atual
  $date = date('Y-m-d');
  // $date = '2025-03-06';
  //
  // $stmt = $conn->prepare("SELECT * FROM classe WHERE data = :date ORDER BY sala, hora_inicio");
  // $stmt->execute(['date' => $date]);
  //
  $stmt = $conn->prepare("SELECT * FROM classe ORDER BY sala, hora_inicio");
  $stmt->execute();
      ...
      <div class="card-header">
        <div class="row align-items-center">
          <div class="col-sm-6 text-sm-start text-center">
            <h5 class="card-title mb-0">Lista de Disponibilidade</h5>
          </div>
          ...
          <table class="table align-middle table_painel">
            ...
            <tbody>
              ...
              foreach ($reservas as $reserva) {
                $salas[$reserva['sala']][] = $reserva;
              }

              foreach ($salas as $sala => $reservas_sala) { ?>
                <tr>
                  <th class="col_cod">
                    <div class="px-2">B3SA304</div>
                  </th>
                  <td>
                    <div class="col_loc" title="<?= htmlspecialchars($sala) ?>"><?= htmlspecialchars($sala) ?></div>
                  </td>
                  <td>
                    <div class="box_s">A2</div>
                  </td>
                  <td>
                    <div class="box_a">P3</div>
                  </td>
                  <td>
                    <div class="box_p">P3</div>
                  </td>
                  <td>
                  ...
                  <?php for ($i = $start; $i <= $end; $i += 1800) {
                    ...
                    foreach ($reservas_sala as $reserva) {
                      ...
                      if ($hora_coluna >= $hora_inicio && $hora_coluna < $hora_fim) {
                        $ocupado = true; ?>
                        <!-- <td class="barra">
                          <?= $hora_resultado = ($hora_coluna == $hora_inicio) ? '<span class="border_l">' . $hora_inicio : ""; ?>
                          <?= ($hora_coluna == date("H:i", strtotime($hora_fim) - 1800) ? '<span class="border_r">' . $hora_fim : "") ?>
                        </td> -->

                        <td class="barra <?= ($hora_coluna == $hora_inicio) ? 'border_l' : ''; ?> <?= ($hora_coluna == date("H:i", strtotime($hora_fim) - 1800)) ? 'border_r' : ''; ?>">
