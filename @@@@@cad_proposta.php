        ...
        <div class="row">
          <div class="col-12 text-truncate">
            <ol class="breadcrumb text-truncate m-0">
              <li class="breadcrumb-item"><a href="painel.php">Solicitação</a></li>
              <?php
              try {
                $propc_cat = $_GET['tp'];
                // $stmt = $conn->prepare("SELECT * FROM propostas_categorias WHERE propc_id = $propc_cat");
                $stmt = $conn->prepare("SELECT * FROM propostas_categorias");
                $stmt->execute();
                while ($propc = $stmt->fetch(PDO::FETCH_ASSOC)) {
                  extract($propc);

                  // CONFIGURAÇÃO DOS ÍCONES DAS CATEGORIAS
                  if ($propc_cat == 1) {
                    $color_card = 'ava_cat_proposta_verde';
                    $icone_card = 'icon_proj_curso_branco.svg';
                  } else if ($propc_cat == 2) {
                    $color_card = 'ava_cat_proposta_laranja';
                    $icone_card = 'icon_proj_evento_branco.svg';
                  } else if ($propc_cat == 3) {
                    $color_card = 'ava_cat_proposta_azul';
                    $icone_card = 'icon_prog_ext_branco.svg';
                  } else if ($propc_cat == 4) {
                    $color_card = 'ava_cat_proposta_roxo';
                    $icone_card = 'icon_parceria_branco.svg';
                  } else if ($propc_cat == 5) {
                    $color_card = 'ava_cat_proposta_rosa';
...
if (isset($_GET['i'])) {
  ...
  if ($result) {
    ...
    $prop_parc_data_atividade_inicial = trim(isset($result['prop_parc_data_atividade_inicial'])) ? $result['prop_parc_data_atividade_inicial'] : NULL;
    ...
    $prop_parc_data_atividade_final   = trim(isset($result['prop_parc_data_atividade_final'])) ? $result['prop_parc_data_atividade_final'] : NULL;
...
<div class="row justify-content-md-center d">
    ...
    <div class="card">
      <div class="card-header border-0 p-sm-4 p-3">
        ...
        if (!empty($_GET['tp'])) { ?>
          ...
          <div id="custom-progress-bar" class="progress-nav step_prop mb-4 d-none">
            ...
            <ul class="nav nav-pills progress-bar-tab custom-nav" role="tablist">
              <li class="nav-item" role="presentation">
                <button onclick="location.href='cad_proposta.php?tp=1&st=1'" class=" nav-link rounded-pill active" data-progressbar="custom-progress-bar" id="pills-gen-info-tab" data-bs-toggle="pill" data-bs-target="#pills-gen-info" type="button" role="tab" aria-controls="pills-gen-info" aria-selected="true" data-position="0">1</button>
              </li>
              <li class="nav-item" role="presentation">
                <button onclick="location.href='cad_proposta.php?tp=1&st=2'" class="nav-link rounded-pill <?= ($_GET['st'] >= 2) ? 'active' : ''; ?>" data-progressbar="custom-progress-bar" id="pills-success-tab" data-bs-toggle="pill" data-bs-target="#pills-success" type="button" role="tab" aria-controls="pills-success" aria-selected="false" data-position="1" tabindex="-1">2</button>
              </li>
              <li class="nav-item" role="presentation">
                <button onclick="location.href='cad_proposta.php?tp=1&st=3'" class="nav-link rounded-pill <?php echo ($_GET['st'] >= 3) ? 'active' : ''; ?>" data-progressbar="custom-progress-bar" id="pills-success-tab" data-bs-toggle="pill" data-bs-target="#pills-success" type="button" role="tab" aria-controls="pills-success" aria-selected="false" data-position="2" tabindex="-1">3</button>
              </li>
              <li class="nav-item" role="presentation">
                <button onclick="location.href='cad_proposta.php?tp=1&st=4'" class="nav-link rounded-pill <?php echo ($_GET['st'] >= 4) ? 'active' : ''; ?>" data-progressbar="custom-progress-bar" id="pills-success-tab" data-bs-toggle="pill" data-bs-target="#pills-success" type="button" role="tab" aria-controls="pills-success" aria-selected="false" data-position="2" tabindex="-1">4</button>
              </li>
              <li class="nav-item" role="presentation">
                <button onclick="location.href='cad_proposta.php?tp=1&st=5'" class="nav-link rounded-pill <?php echo ($_GET['st'] >= 5) ? 'active' : ''; ?>" data-progressbar="custom-progress-bar" id="pills-success-tab" data-bs-toggle="pill" data-bs-target="#pills-success" type="button" role="tab" aria-controls="pills-success" aria-selected="false" data-position="2" tabindex="-1">5</button>
              </li>
              <!-- <li class="nav-item" role="presentation">
                <button onclick="location.href='cad_proposta.php?tp=<?= base64_encode($propc_cat) ?>&st=<?= base64_encode(6) ?>&i=<?= $_GET['i'] ?>'" class="nav-link rounded-pill <?php echo (base64_decode($_GET['st']) >= 6) ? 'active' : ''; ?>" data-progressbar="custom-progress-bar" id="pills-success-tab" data-bs-toggle="pill" data-bs-target="#pills-success" type="button" role="tab" aria-controls="pills-success" aria-selected="false" data-position="2" tabindex="-1" <?php echo ($prop_status_etapa < 5) ? 'disabled' : ''; ?>>6</button>
              </li> -->
            </ul>
          </div>
        ...
        <?php if (!empty($_GET['tp']) && base64_decode($_GET['tp']) != 4) { ?>

          <div class="row tit_cad_proposta d-none">
            <div class="col-12">
              ...
              if (isset($_GET['st'])) {
                ...
                if ($_GET['st'] == 2) {
                  $step_tit = 'Informações da Reserva - Aulas Práticas';
                }
                if ($_GET['st'] == 3) {
                  $step_tit = 'Informações da Reserva - Aulas Teóricas';
                }
                if ($_GET['st'] == 4) {
      ...
      <?php
      if (isset($_GET['tp']) && $_GET['tp'] == 4 || isset($prop_tipo) && $prop_tipo == 4) {
